            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="container-fluid">
                        ...
                        <p style="text-indent: 50px;">ตามที่  {{$item->name_unit}} ได้แจ้งขอรับบริการการตรวจประเมินความสามารถ 
                                    ...
                                    <span style="display: inline-block; width: 805px; border-bottom: 1px dotted #000; margin-left: 10px;">
                                        <span style="opacity: 0.5;">
                                            ...
                                            @if ($item->address_street !== null) ถนน{{$item->address_street}}  @endif
                                                 ...
                                                 @if (strpos($item->BasicProvince->PROVINCE_NAME, 'กรุงเทพ') !== false)
                                                    แขวง {{$item->district_id}} เขต{{$item->amphur_id }} {{$item->BasicProvince->PROVINCE_NAME}}
                                                @else
                                                    ตำบล{{$item->district_id}}  อำเภอ{{$item->amphur_id }}  จังหวัด{{$item->BasicProvince->PROVINCE_NAME}}
                                                @endif 
...
{{--@endif--}}
    ...
    <script>
        ...
        $(document).off('click', '#ability_confirm_button').on('click', '#ability_confirm_button', function(e) {
            ...
            // เรียก AJAX
            $.ajax({
                url: "{{route('applicant-ib.ability-confirm')}}",
                method: "POST",
                data: {
                    _token: _token,
                    id: app_id,
