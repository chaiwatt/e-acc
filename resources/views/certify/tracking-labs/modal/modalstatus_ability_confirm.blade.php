...
{{--@if ($showmodal19 == 19)--}}
    <div class="modal fade text-left" id="report{{$id}}" tabindex="-1" role="dialog" aria-labelledby="addBrand">
        {{-- <div class="modal fade text-left" id="action19" tabindex="-1" role="dialog" aria-labelledby="addBrand"> --}}
        <div class="modal-dialog modal-dialog-centered " style="width:1000px" role="document">
            <div class="modal-content">
                ...
                {{-- {!! Form::open(['url' => 'certify/applicant/update/status/cost/certificate/'.$certificate->id,   'method' => 'POST', 'class' => 'form-horizontal app_certificate_form','files' => true]) !!} --}}
                  {!! Form::open(['url' => 'certify/applicant/update/status/cost/certificate/',   'method' => 'POST', 'class' => 'form-horizontal app_certificate_form','files' => true]) !!}
                @php
                ...
                <div class="modal-body">
                    <div class="container-fluid">
                        ...
                        <p style="text-indent: 50px;">ตามที่  {{ $applicant->information->name }} ได้แจ้งขอรับบริการการตรวจประเมินความสามารถ 
                            ตามมาตรฐาน มอก. {{ !is_null($formula)?$formula->title:'-' }}  ลงรับวันที่  {{ HP::formatDateThaiFull($tracking->tracking_review_to->updated_at) ?? '-' }} </span>นั้น
                        </p>
...
// return;
                ...
                // เรียก AJAX
                fetch("{{route('tracking-lab.ability-confirm')}}", {
                    method: "POST",
                    headers: {
