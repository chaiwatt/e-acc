...
@section('content')
 <div class="container-fluid">
     <div class="row">
        <div class="col-md-12">
           <div class="white-box">
           <h3 class="box-title pull-left">ใบรับรองระบบงานติดตาม (IB) #{{$assessment->id}}</h3>
...
@foreach(collect([$assessment->history_ib_many->last()]) as $key1 => $item)
   ...
   jQuery(document).ready(function() {
    $('#form_auditor').parsley().on('field:validated', function() {
     ...
     $(document).on('click', '#accept_fault', function(e) {
            ...
            // เรียก AJAX
            $.ajax({
                url: "{{route('evaluation.confirm-ib-bug')}}",
                method: "POST",
                data: {
                    _token: _token,
