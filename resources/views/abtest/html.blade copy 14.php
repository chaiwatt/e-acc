...
<body>
    ...
    <div id="lab-cal-item-modal" class="modal-overlay">
        <div class="modal-content">
            ...
            <div class="modal-input-group" style="display: flex; gap: 20px;">
                <div style="flex: 1;">
                    ...
                    <input type="text" id="lab-cal-field" style="font-family: 'thsarabunnew'; font-size:22px;">
                </div>
                <div style="flex: 1;">
                    <label for="lab-cal-instrument">เครื่องมือ:</label>
                    <input type="text" id="lab-cal-instrument" style="font-family: 'thsarabunnew'; font-size:22px;">
                </div>
            </div>
            <div class="modal-input-group" style="display: flex; gap: 20px;">
                <div style="flex: 1;">
                    <label for="lab-cal-parameter">พารามิเตอร์:</label>
                    <input type="text" id="lab-cal-parameter" style="font-family: 'thsarabunnew'; font-size:22px;">
                </div>
                <div style="flex: 1;">
                    <label for="lab-cal-condition">เงื่อนไขการวัด:</label>
                    <input type="text" id="lab-cal-condition" style="font-family: 'thsarabunnew'; font-size:22px;">
                </div>
            </div>
            <div class="modal-input-group" style="display: flex; gap: 20px;">
                <div style="flex: 1;">
                    <label for="lab-cal-param-details-editor">รายละเอียดพารามิเตอร์:</label>
                    <div id="lab-cal-param-details-editor" class="editable-div" style="font-family: 'thsarabunnew'; font-size:22px;" contenteditable="true"></div>
                </div>
                <div style="flex: 1;">
                    <label for="lab-cal-capability-editor">ขีดความสามารถฯ:</label>
                    <div id="lab-cal-capability-editor" class="editable-div" style="font-family: 'thsarabunnew'; font-size:22px;" contenteditable="true"></div>
                </div>
            </div>
            <!-- === END: MODIFICATION === -->
            <div class="modal-input-group">
                <label for="lab-cal-method-editor" style="display: block;">วิธีสอบเทียบ / มาตรฐานที่ใช้:</label>
                <div id="lab-cal-method-editor" class="editable-div" style="font-family: 'thsarabunnew'; font-size:22px;" contenteditable="true"></div>
            </div>
            <div class="modal-buttons">
    ...
    <!-- Lab Test Item Modal -->
    <div id="lab-test-item-modal" class="modal-overlay">
        <div class="modal-content">
            <h3>เพิ่มรายการ (Lab Test)</h3>
            <!-- === START: MODIFICATION === -->
            <div class="modal-input-group">
                <label for="lab-test-field">สาขาการทดสอบ:</label>
                <input type="text" id="lab-test-field" style="font-family: 'thsarabunnew'; font-size:22px;">
            </div>
            <div class="modal-input-group" style="display: flex; gap: 20px;">
                <div style="flex: 1;">
                    <label for="lab-test-parameter-editor">รายการทดสอบ:</label>
                    <div id="lab-test-parameter-editor" class="editable-div" style="font-family: 'thsarabunnew'; font-size:22px;" contenteditable="true"></div>
                </div>
                <div style="flex: 1;">
                    <label for="lab-test-method-editor">วิธีทดสอบ:</label>
                    <div id="lab-test-method-editor" class="editable-div" style="font-family: 'thsarabunnew'; font-size:22px;" contenteditable="true"></div>
                </div>
            </div>
            <!-- === END: MODIFICATION === -->
            <div class="modal-buttons">
...
const insertCbTemplate = () => {
   ...
   const insertIbTemplate = () => {
        ...
        function insertTableRow(table, rowIndex, above = true) {
            ...
            // When inserting BELOW the current row
            if (!above) {
                const rowAbove = table.rows[rowIndex]; // The row we clicked on

                // Remove the top border of the NEW row to connect it to the row above.
                for (const cell of newRow.cells) {
                    cell.style.borderTop = 'none';
                }

                // Remove the bottom border of the ROW ABOVE to connect it to the new row.
                if (rowAbove) {
            ...
            // When inserting ABOVE the current row
            if (above) {
                const rowBelow = table.rows[insertAt]; // The original row, now below the new one

                // Remove the bottom border of the NEW row to connect it to the row below.
                for (const cell of newRow.cells) {
                    cell.style.borderBottom = 'none';
                }

                // Remove the top border of the ROW BELOW to connect it to the new row.
                if (rowBelow) {
        ...
        contextMenu.addEventListener('click', (event) => {
            ...
            switch (action) {
                case 'insert-row-above':    insertTableRow(table, rowIndex, true); break;
                case 'insert-row-below':    insertTableRow(table, rowIndex, false); break;
                case 'insert-column-left':  insertTableColumn(table, colIndex, false); break;
                case 'insert-column-right': insertTableColumn(table, colIndex, true); break;
                case 'delete-row':          deleteTableRow(table, rowIndex); break;
                case 'delete-column':       deleteTableColumn(table, colIndex); break;
            }
        ...
        document.getElementById('add-lab-test-item-btn').addEventListener('click', () => {
            ...
            const parameter = labTestParameterEditorExtractor.getLines().join('<br>');
            const method = labTestMethodEditorExtractor.getLines().join('<br>');
            if (!field && !parameter.trim() && !method.trim()) { closeModal(labTestItemModal); return; }

            const tbody = activeModalTargetRow?.parentElement;
            if (!tbody) { alert("ไม่สามารถหาตารางเป้าหมายได้"); closeModal(labTestItemModal); return; }
        ...
        // Instantiate LineExtractor for all editable divs in the modal
        ...
        const labTestParameterEditorExtractor = new LineExtractor('lab-test-parameter-editor');
