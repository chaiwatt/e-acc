...
<body>
    ...
    <div id="lab-cal-item-modal" class="modal-overlay">
        <div class="modal-content">
            ...
            <div class="modal-input-group">
                <label for="lab-cal-param-details">รายละเอียดพารามิเตอร์:</label>
                <input type="text" id="lab-cal-param-details">
            </div>
            <div class="modal-input-group">
                <label for="lab-cal-capability">ขีดความสามารถฯ:</label>
                <input type="text" id="lab-cal-capability">
            </div>
            <div class="modal-input-group">
                <label for="lab-cal-method">วิธีสอบเทียบ:</label>
                <input type="text" id="lab-cal-method">
            </div>
            <div class="modal-buttons">
    ...
    <div id="lab-test-item-modal" class="modal-overlay">
        <div class="modal-content">
            ...
            <div class="modal-input-group">
                <label for="lab-test-method">วิธีทดสอบ:</label>
                <input type="text" id="lab-test-method">
            </div>
            <div class="modal-buttons">
...
const insertCbTemplate = () => {
   ...
   const insertIbTemplate = () => {
        ...
        // --- Helper function to append text to a cell ---
        function appendToCell(cell, text) {
            if (!text) return; // Do not append empty text
            const cleanHTML = cell.innerHTML.replace(/<br\s*\/?>/gi, '').trim();
            if (cleanHTML === '') {
                cell.innerHTML = text; // If cell is empty or just contains <br>, replace content
            } else {
                cell.innerHTML += '<br>' + text; // Otherwise, append with a line break
            }
        }
        ...
        document.getElementById('add-cb-item-btn').addEventListener('click', () => {
            ...
            if (tbody) {
                ...
                alert("ไม่สามารถหาตารางเป้าหมายได้");
            }
            closeModal(cbItemModal);
        });
        ...
        document.getElementById('add-ib-item-btn').addEventListener('click', () => {
            ...
            if (!category && !procedure && !requirements) {
        ...
        document.getElementById('add-lab-cal-item-btn').addEventListener('click', () => {
            ...
            if (!targetRow) {
                ...
                closeModal(labCalItemModal); // Close modal even if it fails
                return;
            }
            ...
            const field = document.getElementById('lab-cal-field').value;
            const instrument = document.getElementById('lab-cal-instrument').value;
            const parameter = document.getElementById('lab-cal-parameter').value;
            const condition = document.getElementById('lab-cal-condition').value;
            const paramDetails = document.getElementById('lab-cal-param-details').value;
            const capability = document.getElementById('lab-cal-capability').value;
            const method = document.getElementById('lab-cal-method').value;
            
            // Combine fields for the second column
            const parameterColumnContent = [instrument, parameter, condition, paramDetails]
                .filter(Boolean) // Remove empty strings
                .join('<br>');

            // Append data to the cells of the target row
            appendToCell(cells[0], field);
        ...
        document.getElementById('add-lab-test-item-btn').addEventListener('click', () => {
            const field = document.getElementById('lab-test-field').value;
            const parameter = document.getElementById('lab-test-parameter').value;
            const method = document.getElementById('lab-test-method').value;
            if (!field && !parameter && !method) {
